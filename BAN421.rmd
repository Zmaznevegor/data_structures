...
In order to understand the export/import interdependence, we have run the hidden Markov model based on the parameters of Nordic region, Denmark and Sweden. In order to define the differences in fluctuations rather than just changes in the differences between the consumption and productin, we used the log function and diff functions of the production-consumption diffrences over time. It is similar to the applications of the HMM to the market regime analysis, where log and diff functions are applied to the closing price.
...
display.hmm(original = comb,
            model = hmm(param = comb$NORD,
                        data = comb))
```
Above we can see the results based on the parameter of the Nordic countries, which is represented in the data frame `comb` in the column `NORD`. Upper graph shows the actual changes in the state over the time, while thwe bottom graph is displaying the results based on the HMM estimations. Displayed loglikelihood is fairly low, and based on the plots, we can see that model does not provide with the close to the actual results. Noticable, model performs worse for th first 2 years and closer to the actual results in the last year.
...
display.hmm(original = comb,
            model = hmm(param = comb$DK,
                        data = comb))
```
Completely differently model works based on the single-country parameter for Denmark. We can mention that the loglikelihood is even lower that in the previous model. Displayed graph also provides us with understanding of the low dependece for the Norwegian production state and the way it is tied with the Denmark's consumption-to-production variables.  

Unlike the previous model, where the HMM performs poorly for the first part of the perioud and has closer to the actual results towards the end, here we can observe a poor and rather flat performance of the model throughout all the period.
...
display.hmm(original = comb,
            model = hmm(param = comb$SE,
                        data = comb))
```
The final graph shows us surprisingly good results. Loglikelihood is comparable higher than in previous two models. Based on the graph, HMM visibly performs better with minor faults in-between the years. In this case, HMM has grasped the overall trend, having the majority of estimated states being a surplus, which is similar to the actual results.
